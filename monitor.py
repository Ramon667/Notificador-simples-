...
def send_notification(title, url, analysis):
    ...
    if not topic:
        print("[erro] NTFY_TOPIC está vazio.")
        return False
    ...
    try:
        ...
        print(
            f"  [ok] ntfy aceitou a notificação "
            f"(HTTP {response.status_code}, tópico '{topic}')"
        )
        if response.text:
            print(f"  [debug] resposta ntfy: {response.text[:300]}")
        return True

    except requests.RequestException as e:
        print(f"  [erro] falha ao enviar notificação pelo ntfy: {e}")
        if getattr(e, "response", None) is not None:
            print(
                f"  [erro] resposta ntfy: "
...
def send_whatsapp_notification(title, url, analysis):
    if not CALLMEBOT_PHONE or not CALLMEBOT_APIKEY:
        print(
            "[aviso] CALLMEBOT_PHONE/CALLMEBOT_APIKEY "
            "não configurados, pulando WhatsApp."
        )
        return False
    ...
    try:
        ...
        if any(term in lower_response for term in error_terms):
            print(f"  [erro] CallMeBot recusou a mensagem: {response_text[:500]}")
            return False

        print(
            f"  [ok] CallMeBot respondeu HTTP {response.status_code}: "
            f"{response_text[:300]}"
        )
        return True
...
def main():
    ...
    new_count = 0
    pending_notification_failures = 0
    ...
    for title, url in links:
        ...
        if analysis["matched"]:
            ...
            if analysis["course_rows"]:
                print(
                    f"  {len(analysis['course_rows'])} "
                    "linha(s) de curso encontrada(s) na tabela."
                )
            ...
            if not ntfy_sent:
                pending_notification_failures += 1
                print(
                    "  [aviso] o edital não será marcado como concluído, "
                    "pois a notificação principal via ntfy falhou."
                )
                continue
        else:
    ...
    save_seen(seen)
    print(f"Estado salvo em {SEEN_FILE}.")

    if pending_notification_failures:
        print(
            f"[aviso] {pending_notification_failures} edital(is) com match "
            "ficaram pendentes para nova tentativa de notificação."
        )
